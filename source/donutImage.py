...
class donutImage(object):
    ...
    def makeMask(self, inst, boundaryT, maskScalingFactor ):
        ...
        rMask = inst.apertureDiameter / (2 * inst.focalLength/inst.offset)*maskScalingFactor

        for ii in range(self.masklist.shape[0]):

            r = np.sqrt( (inst.xSensor-self.masklist[ii,0])**2 + (inst.ySensor-self.masklist[ii,1])**2 )

            # Initialize both mask elements to the opposite of the pass/block boolean
            pMaskii = (1-self.masklist[ii,3])*np.ones( (inst.sensorSamples,inst.sensorSamples), dtype=int )
            cMaskii = (1-self.masklist[ii,3])*np.ones( (inst.sensorSamples,inst.sensorSamples), dtype=int )
            ...
            if (self.masklist[ii,3] >= 1):
                ...
                aidx = np.nonzero(r <= self.masklist[ii,2]*(1+boundaryT*inst.pixelSize/rMask)) 
            else:
                aidx = np.nonzero(r <= self.masklist[ii,2]*(1-boundaryT*inst.pixelSize/rMask))
            ...
            self.pMask = self.pMask*pMaskii    #padded mask - for use at the offset planes
            self.cMask = self.cMask*cMaskii    #non-padded mask corresponding to aperture

            
    def getOffAxisCorr(self,order):
        self.offAxis_coeff = np.zeros((4,(order+1)*(order+2)/2))
        self.offAxis_coeff[0,:] = getOffAxisCorr_single('data/lsst/offAxis_cxin_poly%d.txt'%(order),self.fldr)
        self.offAxis_coeff[1,:] = getOffAxisCorr_single('data/lsst/offAxis_cyin_poly%d.txt'%(order),self.fldr)
        self.offAxis_coeff[2,:] = getOffAxisCorr_single('data/lsst/offAxis_cxex_poly%d.txt'%(order),self.fldr)
        self.offAxis_coeff[3,:] = getOffAxisCorr_single('data/lsst/offAxis_cyex_poly%d.txt'%(order),self.fldr)
    ...
    def imageCoCenter(self,inst,algo):
        ...
        radialShift=3.5*algo.upReso*(inst.offset/1e-3)*(10e-6/inst.pixelSize)
    ...
    def compensate(self, inst,algo,zcCol, oversample,model):
        ...
        #set up the mapping
        lutxp, lutyp, J = aperture2image(self,inst,algo,zcCol,lutx,luty,projSamples,model)
        #    print "J",J.shape
...
def aperture2image(Im,inst,algo,zcCol,lutx,luty,projSamples,model):
    ...
    if (Im.type == 'intra'):
        myC = - inst.focalLength*(inst.focalLength-inst.offset)/inst.offset/R**2
    elif (Im.type == 'extra'):
    ...
    #print "LUTR",lutr.shape,((lutr >1 + onepixel) | (lutr<inst.obscuration-onepixel)).shape
    ...
    idxinbd=((lutr<inst.obscuration) & (lutr>inst.obscuration-onepixel)) #inner boundary 
    ...
    if (model == 'paraxial'):
        ...
        myA=np.sqrt((inst.focalLength**2-R**2)/(inst.focalLength**2-lutr**2*R**2))
        ...
        if (Im.type == 'intra'):
            cx=(Im.offAxis_coeff[0,:]-Im.offAxis_coeff[2,:])*(1e-3+inst.offset)/2e-3+Im.offAxis_coeff[2,:]
            cy=(Im.offAxis_coeff[1,:]-Im.offAxis_coeff[3,:])*(1e-3+inst.offset)/2e-3+Im.offAxis_coeff[3,:]
        elif (Im.type == 'extra'):
            cx=(Im.offAxis_coeff[0,:]-Im.offAxis_coeff[2,:])*(1e-3-inst.offset)/2e-3+Im.offAxis_coeff[2,:]
            cy=(Im.offAxis_coeff[1,:]-Im.offAxis_coeff[3,:])*(1e-3-inst.offset)/2e-3+Im.offAxis_coeff[3,:]
        ...
        reduced_coordi_factor = 1/(inst.sensorSamples/2*inst.pixelSize/inst.sensorFactor)/1000; #Zemax data are in mm, therefore 1000
    ...
    if (zcCol.ndim ==1):
        if (algo.zobsR>0):
            lutxp =lutxp + myC* ZernikeAnnularGrad(zcCol,lutx, luty,algo.zobsR,'dx')
            lutyp =lutyp + myC* ZernikeAnnularGrad(zcCol,lutx, luty,algo.zobsR,'dy')
        else:
            lutxp = lutxp + myC * ZernikeGrad(zcCol,lutx, luty,'dx')
            lutyp = lutyp + myC * ZernikeGrad(zcCol,lutx, luty,'dy')
    else:
        FX, FY=gradient(zcCol,inst.sensorFactor/(inst.sensorSamples/2))
        lutxp =lutxp + myC* FX
        lutyp =lutyp + myC* FY
    ...
    if (zcCol.ndim ==1):
        if (model == 'paraxial'):
            if (algo.zobsR>0):
                J= (1+ myC * ZernikeAnnularJacobian(zcCol, lutx, luty,algo.zobsR, '1st') + 
                    myC**2 * ZernikeAnnularJacobian(zcCol, lutx, luty,algo.zobsR, '2nd'))
            else:
                J= (1+ myC * ZernikeJacobian(zcCol, lutx, luty, '1st') + 
                    myC**2 * ZernikeJacobian(zcCol, lutx, luty, '2nd'))

        elif (model == 'onAxis'):
            xpox = algo.maskScalingFactor*myA*(1+lutx**2*R**2./(inst.focalLength**2-R**2*lutr**2)) + \
                   myC * ZernikeAnnularGrad(zcCol,lutx, luty,algo.zobsR,'dx2')
            ypoy = algo.maskScalingFactor*myA*(1+luty**2*R**2./(inst.focalLength**2-R**2*lutr**2)) \
                   +myC* ZernikeAnnularGrad(zcCol,lutx, luty,algo.zobsR,'dy2')
            xpoy = algo.maskScalingFactor*myA*lutx*luty*R**2/(inst.focalLength**2-R**2*lutr**2) \
            ...
            ypoy = (xp0oy*sintheta+yp0oy*costheta)*reduced_coordi_factor \
                   +myC* ZernikeAnnularGrad(zcCol,lutx, luty,algo.zobsR,'dy2')
            temp=myC*ZernikeAnnularGrad(zcCol,lutx, luty,algo.zobsR,'dxy')
        ...
        FXX, FXY =gradient(FX,inst.sensorFactor/(inst.sensorSamples/2))
        tmp, FYY =gradient(FY,inst.sensorFactor/(inst.sensorSamples/2))
        if (model == 'paraxial'):
            xpox = 1+myC* FXX
            ypoy = 1+myC* FYY
            xpoy = 1+myC* FXY
            ...
            xpox = algo.maskScalingFactor*myA*(1+lutx**2*R**2./(inst.focalLength**2-R**2*lutr**2)) +myC* FXX
            ypoy = algo.maskScalingFactor*myA*(1+luty**2*R**2/(inst.focalLength**2-R**2*lutr**2)) +myC* FYY
            xpoy = algo.maskScalingFactor*myA*lutx*luty*R**2./(inst.focalLength**2-R**2*lutr**2)   +myC* FXY
            ypox = xpoy
        elif (model == 'offAxis'):

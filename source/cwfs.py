...
def main():
    ...
    parser = argparse.ArgumentParser(description=
                                     '-----This is cwfs (Curvature Wavefront Sensing) code----')
    ...
    parser.add_argument('-dir', dest='imgDir',help='relative or absolute path for input images')
    parser.add_argument('-ixy',dest='intra_xy', nargs=2,type=float,default=[0,0],
                        help='intra focal field (x,y) in deg, default=[0 0]')
    parser.add_argument('-exy',dest='extra_xy', nargs=2,type=float,default=[0,0],
                        help='extra focal field (x,y) in deg, default=[0 0]')
    parser.add_argument('-i', dest='instruFile',default='lsst',
                        help='instrument parameter file, default=lsst, ".param" is appended automatically \
                        default path is data/lsst/')
    parser.add_argument('-a', dest='algoFile',default='fft',
                        help='algorithm parameter file, default=fft, ".algo" is appended automatically\
                        default path is data/algo/')
    parser.add_argument('-m', dest='model',choices=('paraxial', 'onAxis','offAxis'),
                        default='paraxial', help='Optical model to be used, default=paraxial')
    parser.add_argument('-op', dest='outputParam',default='',
                        help='file name for dumping all parameters, default=no output')
    parser.add_argument('-oz', dest='outputZer',default='',
                        help='file name for output Zernikes (in unit of nm), default=no output')
    parser.add_argument('-v','--version', action='version', version='%(prog)s 1.0')
    parser.add_argument('-d',dest='debug_level', type=int, default=0,choices=(-1,0,1,2,3),
    ...
    I1=donutImage(intraFile,args.intra_xy,'intra')
    I2=donutImage(extraFile,args.extra_xy,'extra')
